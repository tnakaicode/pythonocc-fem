...
#
# <i>Note that the following notebook is very compute intensive.
# To speed it up disable the near-field computations.</i>
#
# In this notebook,
# we consider the bistatic radar cross section generated from plane wave scattering by an array of dielectric spheres $\Omega_j$,
# each with its individual permittivity $\epsilon_j$ and permeability $\mu_j$.
#
# We denote by $\epsilon_{0}$ and $\mu_0$ the electric permittivity and magnetic permeability in vacuum,
# and by $\epsilon_{r, j} = \frac{\epsilon_j}{\epsilon_0}$ and $\mu_{r, j} = \frac{\mu_j}{\mu_0}$ the relative permittivity and relative permeability for each dielectric object. Denote by $\mathbf{E}^\text{s}$, $\mathbf{H}^\text{s}$
# the scattered electric and magnetic field in the exterior of the scatterers. Moreover,
# let $\mathbf{E}$ and $\mathbf{H}$ be the total exterior fields.
# Correspondingly, we denote the interior fields in the $j$th obstacle by $\mathbf{E}_j$ and $\mathbf{H}_j$.
#
...
grids = [bempp.api.shapes.sphere(
    r=radius, origin=(c, 0, 0), h=0.1) for c in centers]
