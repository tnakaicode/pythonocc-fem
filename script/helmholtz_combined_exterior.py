...
# A boundary operator always takes at least three space arguments:
# a domain space, a range space and the test space (dual to the range).
# In this example we only work on the space $\mathcal{L}^2(\Gamma)$ and
# we can choose all spaces to be identical.
...
# We now form the right-hand side by defining
# a <a href='https://bempp.com/2017/07/11/grid-functions/'>GridFunction</a> using Python callable.
...
# `gmres` returns a grid function `neumann_fun` and an integer `info`.
# When everything works fine info is equal to 0.
#
# At this stage, we have the surface solution of the integral equation.
# Now we will evaluate the solution in the domain of interest.
# We define the evaluation points as follows.
...
# The variable ``idx`` allows to compute the solution only at points located outside the unit circle of the plane.
# We use a single layer potential operator to evaluate the solution at the observation points.
...
obj.new_2Dfig()
img = obj.axs.imshow(np.real(u_evaluated.T), extent=[-3, 3, -3, 3], cmap="jet")
obj.axs.set_xlabel('x')
obj.axs.set_ylabel('y')
obj.fig.colorbar(img)
obj.axs.set_title("Scattering from the unit sphere, solution in plane z=0")
obj.SavePng()
