...
class ShapeSlice(dispocc):
    ...
    def run(self, n_procs, compare_by_number_of_processors=False):
        shape = self.get_brep()
        ...
        def get_z_coords_for_n_procs(n_slices, n_procs):
            z_slices = self.drange(z_min, z_max, dz / n_slices)
        ...
        if not compare_by_number_of_processors:
            ...
            _results = P.map(vectorized_slicer,
                             arguments(n_slice, n_procs))
            ...
            for i in range(1, 9):
                ...
                if i == 1:
                    _results = vectorized_slicer(
                        [self.drange(z_min, z_max, dz / n_slice), shape])
                else:
                    P = multiprocessing.Pool(n_procs)
                    _results = P.map(vectorized_slicer,
                                     arguments(n_slice, i))
        ...
        for n, result_shp in enumerate(_results):
            ...
            self.display.DisplayShape(result_shp, update=True)
...
if __name__ == '__main__':
    ...
    nprocs = multiprocessing.cpu_count()
    obj = ShapeSlice()
    obj.run(nprocs, compare_by_number_of_processors=False)
