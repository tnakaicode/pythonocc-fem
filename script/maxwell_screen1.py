...
# Background
#
# In this tutorial, we consider the scattering of an electromagnetic wave from a collection of perfectly conducting screens.
# The time-harmonic Maxwell equation for the electric field $\mathbf{E}$ reduces to
#
# $$
# \nabla\times\nabla\times \mathbf{E} -k^2 \mathbf{E} = 0
# $$
#
# in $\mathbb{R}^3\backslash\Gamma$,
# where $k:=2\pi/\lambda$ is the wavenumber,
# $\lambda$ is the wavelength, and
# $\Gamma$ denotes the screens.
#
# The electric field $\mathbf{E}$ is ]
# the sum of the incident field $\mathbf{E}^\text{inc}$ and
# the scattered field $\mathbf{E}^\text{s}$.
#
# Here, we use the incident field given by
#
# $$
# \mathbf{E}^\text{inc}(\mathbf{x}):=\begin{bmatrix} \mathrm{e}^{\mathrm{i}kz} & 0 & 0 \end{bmatrix},
# $$
#
# which is a wave travelling in the $z$ direction and polarised in the $x$ direction.
# On the screen, the tangential component $\mathbf{E}_\text{t}:=\nu\times \mathbf{E}$ must be zero.
# Towards infinity we impose the Silver&ndash;M&uuml;ller radiation condition
#
# $$
...
# $$
#
# where $\Lambda$ is the jump of the Neumann trace of the scattered field $\mathbf{E}^\text{s}$ across the screen.
# The <a href='https://bempp.com/2017/07/11/available_operators/'>Maxwell electric field potential operator</a> $\mathcal{E}$ is defined as
#
# $$
...
#
# The associated boundary operator is denoted by $\mathsf{E}$.
# It is defined as average tangential trace of the electric field potential operator from both sides of the screen.
# The boundary integral equation is now
#
# $$
# \mathsf{E}\Lambda = \nu\times \mathbf{E}^\text{inc}.
# $$
#
# The $-$ sign is missing in comparison to the representation formula since
# we want to satisfy the boundary conditions for the negative incident wave
# so that the tangential trace of the total field is zero on the screen.
#
...
# Next, we define the Maxwell electric field boundary operator and the identity operator.
# For Maxwell problems, the ``domain`` and ``range`` spaces should be div-conforming, while the ``dual_to_range`` space should be curl conforming.
...
# In addition, we define a Python callable with the incident field, which is later used for plotting.
...
# Now that the solution $\mathbf{\Lambda}$ is computed,
# we want to plot the total field. First, we define a grid of points in the $x$-$z$ plane.
...
# The following commands now compute the total field
# by first computing the scattered field from the representation formula then adding the incident field.
...
# In electromagnetic scattering it is often useful to visualise the squared electric field density.
...
obj = plot2d()
img = obj.axs.imshow(squared_field_density.reshape((nx, nz)).T,
                     cmap='jet', origin='lower',
                     extent=[-extent, extent, -extent, extent], vmax=4)
obj.axs.plot([-.5, -2], [0, 2], 'k-', linewidth=4)
obj.axs.plot([.5, 2], [0, 2], 'k-', linewidth=4)
obj.axs.plot([-1, 1], [-1, -1], 'k-', linewidth=4)


obj.fig.colorbar(img, ax=obj.axs)
obj.axs.set_title("Squared Electric Field Density")
obj.SavePng()
