...
# Background
#
# In this tutorial, we will solve a simple Laplace problem inside the unit sphere with Dirichlet boundary conditions.
# Let $\Omega$ be the unit sphere with boundary $\Gamma$. Let $\nu$ be the outward pointing normal on $\Gamma$.
...
# $$
#
# be the Green's function for Laplace in three dimensions.
# From Green's representation theorem, it follows that every harmonic function $u$ in $\Omega$ satisfies
#
# $$
...
print(bempp.api.TMP_PATH)
print(bempp.api.GMSH_PATH)
grid = bempp.api.shapes.sphere(h=0.1)


# We now define the <a href='https://bempp.com/2017/07/11/function-spaces/'>spaces</a>.
# For this example we will use two spaces: the space of continuous, piecewise linear functions;
# and the space of piecewise constant functions.
...
# We can now define the <a href='https://bempp.com/2017/07/11/operators/'>operators</a>.
# We need the identity, single layer, and double layer boundary operator.
...
# We now define the <a href='https://bempp.com/2017/07/11/grid-functions/'>GridFunction object</a> on the sphere grid
# that represents the Dirichlet data.
...
# We next assemble the right-hand side of the boundary integral equation,
# given by $$(\tfrac12\mathsf{Id}+\mathsf{K})u.$$


rhs = (.5 * identity + dlp) * dirichlet_fun


# We now <a href='https://bempp.com/2017/07/12/solving-linear-systems/'>solve the linear system</a>
# using a conjugate gradient (CG) method.


neumann_fun, info = bempp.api.linalg.cg(slp, rhs, tol=1E-3)


# We now want to provide a simple plot of the solution in the $(x,y)$ plane for $z=0$.
# We first define points at which to plot the solution.
...
# The variable `points` now contains in its columns the coordinates of the evaluation points.
# We can now use Green's representation theorem to evaluate the solution on these points.
#  Note in particular the last line of the following code. It is a direct implementation of Green's representation theorem.
...
# We now plot the 2D slice of the solution.
# For a full three dimensional visualization,
# Bempp can <a href='https://bempp.com/2017/07/12/import-and-export-of-gmsh-files/'>export the data to Gmsh</a>.
# Since the solution decays quickly, we use a logarithmic plot.
...
obj.new_2Dfig()
img = obj.axs.imshow(np.log(np.abs(u_evaluated.T)),
                     extent=(-1, 1, -1, 1), cmap="jet")
obj.axs.set_title('Computed solution')
